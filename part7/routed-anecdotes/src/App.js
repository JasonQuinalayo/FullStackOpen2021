...
const CreateNew = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (props.notifTimeoutId) clearTimeout(props.notifTimeoutId)
    props.setNotification(`You added ${content.value}.`)
    props.setNotifTimeoutId(
      setTimeout(
        () => {
          props.setNotification('')
          props.setNotifTimeoutId(null)
        },
        10000
      )
    )
...
const Anecdote = ({ anecdote }) => {
  return (
    <div>
      {Object.keys(anecdote).map(key => (
        <div key={key}>
          <span><strong>{key}</strong>:{anecdote[key]}</span><br/>
        </div>
      ))}
    </div>
  )
      ...
      <Switch>
        ...
        <Route exact path="/create">
          <CreateNew 
            ...
            notifTimeoutId={notifTimeoutId}
            setNotifTimeoutId={setNotifTimeoutId}
          />
        </Route>
      </Switch>
      <Footer />
